...
/*   Updated: 2026/08/24 14:24:13 by pilagach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char    *e_redirbuilder(t_command *cmd)
{
	...
	cwd = getcwd(NULL, 0);
	path = (char *)ft_calloc(sizeof(char), (ft_strlen(cwd) + ft_strlen(cmd->redir->file) + 2));
	ft_strcat(path, cwd);
	ft_strcat(path, "/");
...
int	e_redir(t_command *cmd)
{
	...
	if (cmd->redir->type == TOKEN_REDIR_OUT)
		fdout = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	else if (cmd->redir->type == TOKEN_APPEND)
...
//echo pouet | cat -e >> korone| grep "pouet"